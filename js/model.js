...
{id:'white',label:'흰색',hex:'#f4f7ff'},
{id:'yellow',label:'노란색',hex:'#ffe49a'},
{id:'orange',label:'주황색',hex:'#ffbb83'},
{id:'red',label:'붉은색',hex:'#ff929c'},
{id:'blue',label:'푸른색',hex:'#99cdff'}
].map(Object.freeze));
const starColor=s=>STAR_COLORS.find(c=>c.id===s.color)||STAR_COLORS[0];
...
function addStar(d,x,y,size=1,brightness=1,color='white'){if(d.stars.length>=MAX_STARS)return null;const id='s'+d.nextId++;d.stars.push({id,...point(x,y),size:clamp(size,0,2),brightness:clamp(brightness,0,2),color:starColor({color}).id});return id}
...
const API={STAR_COLORS,starColor,WIDTH,HEIGHT,MAX_STARS,MAX_STROKES,copy,clamp,blank,point,addStar,moveStar,removeStar,connect,largestGroup,normalize,distanceToSegment,nearestStar,eraseStroke,backgroundStars};
